...
const Navbar = () => {
  ...
  return (
    <nav ref={navRef} className="relative w-full flex justify-between md:justify-center items-center pl-0 pr-4 md:px-8 lg:px-16 py-10">
      {/* Logo for mobile */}
      ...
      {/* Mobile menu overlay */}
      {isOpen && (
        <div className="md:hidden absolute top-full left-50 w-full bg-black bg-opacity-90 flex flex-col items-center py-8 z-20">
          {navLinks.map((link) => (
            <Link
              key={link.name}
              href={link.href}
